...
from PySide6.QtWidgets import QPlainTextEdit, QPushButton, QTableView
...
def test_selection_logic():
    ...
    assert model.get_selected_ids() == ["Id1"]
...
def test_normal_update_failure_retries_without_silent(qtbot):
    window = MainWindow()
    qtbot.addWidget(window)
    window.current_operation = "update"
    ...
    assert window.process_queue[0]["retried_without_silent"] is True
